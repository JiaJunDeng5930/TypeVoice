...
impl ContextService {
    ...
    pub fn capture_hotkey_context_now(
        ...
        if !cfg.include_prev_window_screenshot {
            let e = anyhow!(
                "E_CONTEXT_SCREENSHOT_DISABLED: context_include_prev_window_screenshot=false"
            );
            span.err_anyhow("config", "E_CONTEXT_SCREENSHOT_DISABLED", &e, None);
            return Err(e);
        }
        ...
        let snapshot = ContextSnapshot {
            recent_history: vec![],
            clipboard_text: None,
            prev_window: Some(crate::context_pack::PrevWindowInfo {
