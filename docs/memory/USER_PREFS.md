...
- 当用户明确要求“严格按文档命令执行”时，执行纪律为“零偏移禁制”：
  - 只能执行文档原文命令；参数、顺序、工作目录、调用方式不得改写。
  - 禁止额外包裹脚本、禁止插入前后置步骤、禁止等价替换命令。
  - 命令失败时必须先原样回传失败命令与原始报错并停止。
  - 若文档给出修复命令，只能执行文档给出的修复命令后重跑同一命令。
  - 若文档未给出下一步，必须停止并等待用户指令，禁止自行扩展排障（包括先看脚本实现）。
- 未经用户明确许可，严禁删除或清空本地环境关键产物（包括但不限于 `.venv/`、`models/`、`apps/desktop/node_modules/`、`apps/desktop/src-tauri/toolchain/bin/`）；若需要清理，必须先获得用户逐项确认。
