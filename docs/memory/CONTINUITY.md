...
- [VERIFIED] 已定位 `recording session not found` 根因：PTT 长按期间出现键盘自动重复 `Pressed`，后端重复 `open_recording_session` 时触发 60 秒 orphan 清理，误删首个待消费会话；证据链见 `D:\Projects\TypeVoice\tmp\typevoice-data\trace.jsonl`（`CMD.start_backend_recording -> CTX.hotkey_capture_now(重复 Pressed) -> CMD.stop_backend_recording -> CMD.start_task(E_START_TASK_FAILED)`）。
- [UNCONFIRMED] 已修复 PTT 重复 `Pressed` 去重：`hotkeys.rs` 对“按住期间重复 Pressed”直接忽略，避免重复开会话并触发误清理；同时 `recording session not found/in use` 改为显式错误码 `E_RECORDING_SESSION_NOT_FOUND`/`E_RECORDING_SESSION_IN_USE`，前端不再回退 `E_UNKNOWN`。
