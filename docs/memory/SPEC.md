...
- Remote ASR 协议：默认 `POST https://api.server/transcribe`（可配置 URL），`Authorization: Bearer <api_key>`，`multipart/form-data` 必填 `file`，可选 `model`，响应 JSON 核心字段 `text`。
- Remote ASR 并发：以音频切片并发转写，支持设置并发数（配置项 `remote_asr_concurrency`，范围 `1..16`，默认 `4`）。
- LLM 改写：仅在用户启用时发送转录文本与必要上下文，不上传音频；当前上下文固定为文本结构，不发送窗口截图。
