...
### 5.4 Rewrite 上下文采集（冻结）

- 改写上下文固定为文本结构，不发送截图或其他图像输入。
- `context_capture_mode` 语义固定为：
  - `minimal`：默认只保留元信息 / 历史 / 剪贴板，不默认采集输入状态、相关文本、可见文本。
  - `balanced`：默认采集输入状态与相关文本，但不默认采集整窗可见文本。
  - `full`：默认采集输入状态、相关文本与整窗可见文本。
  - app/domain `deny` 优先级高于 `allow`，且会同时抑制输入状态、相关文本、可见文本；显式 `allow` 可覆盖对应模式的默认关闭。
  - app/domain 规则必须在正文文本采集前生效；被 `deny` 的目标不允许先读再丢弃。
- Windows 上下文采集以当前前台目标的可访问性文本为主，至少可包含：
  - 焦点应用 / 窗口元信息
  - 焦点元素元信息
  - 输入状态（选区、前文、后文、全文）
  - 焦点周边相关文本
  ...
  - 最近历史片段
  - 剪贴板文本（可配置）
- 上下文能力必须全部纳入 settings 与设置页管理，不允许保留用户不可见的截图/vision 开关。
- 热键与主 UI 路径都必须在录音开始时冻结同一份上下文快照，`Rewrite` 阶段只消费该快照，不重新采样外部 UI 状态。
- 若录音开始时尚不存在 `settings.json`，录音链路仍应使用默认 rewrite context 配置完成冻结，不能因此阻断录音启动。
