...
- 连续性：连续完成 50 次“录音 -> 预处理 -> 转录 -> 可选改写 -> 复制”流程无崩溃、无卡死
...
若未达标，必须在进入“功能开发”前完成性能优化或降级策略的决策（见 `docs/perf-spike-plan-v0.1.md`）。

### 5.3 资源占用与泄漏（MVP 验收）

- 连续执行 50 次任务后，常驻内存不得呈线性增长趋势。
- GPU 推理优先；若 GPU 不可用允许降级到 CPU，但必须在 UI 显示“CPU 模式”并提示速度可能变慢。
