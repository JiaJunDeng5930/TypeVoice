...
面向平台：Windows 桌面端为主（自动粘贴能力需兼容 Windows、Linux 与 macOS）
...
- 一键复制最终文本到剪贴板
- 自动粘贴（默认开启，可在设置中关闭）
- 自动粘贴实现不得通过快捷键模拟（如 `Ctrl+V`），必须使用平台 API
- 自动粘贴在 Linux、Windows 与 macOS 均需可用
- 历史记录（冻结选择）
- 仅保存文本与元信息
- 不保存音频
...
- 历史记录仅保存文本，不保存音频。
- 一键复制工作正常。
- 自动粘贴在 Linux、Windows 与 macOS 上工作正常，且失败时展示结构化错误提示。
